...
export default function App() {
  ...
  // ================= NOTES SCREEN =================
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-slate-900 to-black text-white p-8">
      ...
      <div className="max-w-xl mx-auto mt-6 space-y-4">
        {notes.length === 0 ? (
          ...
          notes.map((n) => (
            <div
              key={n._id}
              className="bg-white/10 backdrop-blur-lg border border-white/20 p-4 rounded shadow"
            >
              <h2 className="font-bold text-lg">{n.title}</h2>
              <p>{n.content}</p>

              <p className="italic text-sm text-gray-400 mt-2">
                AI Summary: {summarize(n.content)}
              </p>
